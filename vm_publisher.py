...
def on_press(key):
    ...
    if k == 'w':
        ...
        client.publish("rachelan/lcd", "w")
    elif k == 'a':
        # send "a" character to rpi
        client.publish("rachelan/lcd", "a")
        #send "LED_ON"
        client.publish("rachelan/led", "LED_ON")
    elif k == 's':
        # send "s" character to rpi
        client.publish("rachelan/lcd", "s")
    elif k == 'd':
        # send "d" character to rpi
        client.publish("rachelan/lcd", "d")
        # send "LED_OFF"
        client.publish("rachelan/led", "LED_OFF")
